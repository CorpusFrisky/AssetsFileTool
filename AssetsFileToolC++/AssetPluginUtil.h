...
struct AssetUtilDesc
{
	...
	std::string makeExportFilePath(std::unordered_map<std::string, size_t>& nameCountBuffer,
...
public:
	...
	AssetTypeTemplateField& getTemplateField(class AppContext& appContext, class AssetIdentifier& asset,
...
public:
	...
	AssetExportTask(std::vector<AssetUtilDesc> assets, std::string taskName,
	...
	virtual bool exportAsset(AssetUtilDesc& asset, std::string path) = 0;
	virtual void onCompletion(const std::string &outputPath);
};

class AssetExportRawTask : public AssetExportTask
{
public:
	AssetExportRawTask(std::vector<AssetUtilDesc> assets, std::string taskName,
		std::string extension, std::string baseDir = "", bool stopOnError = false);
	inline ~AssetExportRawTask() noexcept(true) {}
	bool exportAsset(AssetUtilDesc& desc, std::string path);
};
...
public:
	AssetExportDumpTask(class AppContext& appContext,
...
public:
	AssetExportTextDumpTask(class AppContext& appContext,
		std::vector<AssetUtilDesc> assets, std::string taskName,
		std::string extension, std::string baseDir = "", bool stopOnError = false);
};
...
class AssetExportJSONDumpTask : public AssetExportDumpTask
{
	...
	void dumpAsset(IAssetsReader* pReader, AssetTypeValueField* pBaseField, IAssetsWriter* pDumpWriter);
public:
	AssetExportJSONDumpTask(class AppContext& appContext,
		std::vector<AssetUtilDesc> assets, std::string taskName,
		std::string extension, std::string baseDir = "", bool stopOnError = false);
};
...
//	}
//public:
//	CGenericBatchImportDialogDesc(std::vector<AssetUtilDesc> _elements, const std::string& extensionRegex);
...
//	AppContext& appContext;
//public:
//	AssetImportDumpTask(AppContext& appContext,
//		std::vector<AssetUtilDesc> assets, std::vector<std::string> importFilePaths,
//		std::string taskName, bool stopOnError = false);
//	bool importAsset(AssetUtilDesc& asset, std::string path, std::optional<std::reference_wrapper<TaskProgressManager>> progressManager = {});
//
//	void parseTextDump(IAssetsReader* pDumpReader, IAssetsWriter* pWriter);
//	void parseJSONDump(IAssetsReader* pDumpReader, IAssetsWriter* pWriter);
//	void parseDump(IAssetsReader* pDumpReader, IAssetsWriter* pWriter);
//};
