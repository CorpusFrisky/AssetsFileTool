...
public:
	...
	{}
	AssetIdentifier(unsigned int fileID, std::shared_ptr<AssetsEntryReplacer> &pReplacer);
	//Instantiate an AssetIdentifier with a relative fileID relative to the dependencies for an absolute fileID and a pathID.
	AssetIdentifier(unsigned int referenceFromFileID, unsigned int relFileID, pathid_t pathID);
	//Instantiate an AssetIdentifier with a relative fileID (from pReferenceFromFile->references) and a pathID.
	AssetIdentifier(std::shared_ptr<class AssetsFileContextInfo> &pReferenceFromFile, unsigned int relFileID, pathid_t pathID);
	AssetIdentifier(std::shared_ptr<class AssetsFileContextInfo> pFile, pathid_t pathID);
	AssetIdentifier(std::shared_ptr<class AssetsFileContextInfo> pFile, AssetFileInfoEx *pAssetInfo);
	AssetIdentifier(std::shared_ptr<class AssetsFileContextInfo> pFile, std::shared_ptr<AssetsEntryReplacer> &pReplacer);
	bool resolve(class AppContext &appContext);
	...
	size_t read(size_t size, void *buffer); //Returns the actually read bytes.
...
public:
	...
	AssetIterator(class AssetsFileContextInfo *pContextInfo, bool ignoreExisting = false, bool ignoreReplacers = false, bool ignoreRemoverReplacers = true);
	~AssetIterator();
	inline AssetIterator(const AssetIterator &other) { (*this) = other; }
	AssetIterator(AssetIterator &&other);
	AssetIterator &operator=(const AssetIterator &other);
	...
	void get(AssetIdentifier &identifier);
};
