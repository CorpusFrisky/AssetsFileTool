...
public:
	FileContextInfo(unsigned int fileID, unsigned int parentFileID);
	virtual ~FileContextInfo();
	virtual IFileContext *getFileContext()=0;
	virtual void getChildFileIDs(std::vector<unsigned int> &childFileIDs)=0;
	virtual void onCloseChild(unsigned int childFileID)=0;
	virtual bool hasNewChanges(class AppContext &appContext)=0;
	virtual bool hasAnyChanges(class AppContext &appContext)=0;
	virtual uint64_t write(class AppContext &appContext, IAssetsWriter *pWriter, uint64_t start, bool resetChangedFlag)=0;
	virtual std::unique_ptr<BundleReplacer> makeBundleReplacer(class AppContext &appContext, const char *oldName, const char *newName, uint32_t bundleIndex, bool resetChangedFlag)=0;
...
class AssetsFileContextInfo : public FileContextInfo
{
	...
	std::vector<std::shared_ptr<ClassDatabaseFile>> &lockScriptDatabases();
	void unlockScriptDatabases();
	...
	void addReplacer(std::shared_ptr<AssetsEntryReplacer> replacer, class AppContext &appContext, bool reuseTypeMetaFromOldReplacer, bool signalMainThread);

public:
	AssetsFileContextInfo(AssetsFileContext *pContext, unsigned int fileID, unsigned int parentFileID);
	...
	std::vector<std::shared_ptr<AssetsReplacer>> getAllReplacers();
	std::shared_ptr<AssetsEntryReplacer> getReplacer(pathid_t pathID);
	//Adds a replacer for an asset. Removes any previous replacers for that path ID. 
	//If reuseTypeMetaFromOldReplacer is set and the replacer is not a remover, 
	// type information from any previous replacer is transferred to the new one.
	...
	int32_t GetClassByName(const char *name);
	...
	std::string GetClassName_(class AppContext &appContext, int32_t classID, uint16_t scriptIndex = 0xFFFF, class AssetIdentifier *pAsset = nullptr);
	bool MakeTemplateField(AssetTypeTemplateField* pTemplateBase, class AppContext& appContext, int32_t classID, uint16_t scriptIndex = 0xFFFF, class AssetIdentifier* pAsset = nullptr,
		std::optional<std::reference_wrapper<bool>> missingScriptTypeInfo = {});
	bool FindScriptClassDatabaseEntry(ClassDatabaseFile *&pClassFile, ClassDatabaseType *&pClassType, class AssetIdentifier &asset, class AppContext &appContext, Hash128 *pScriptID = nullptr);
	...
	bool FindClassDatabase(ClassDatabasePackage &package);
	//Sets the class database.
	void SetClassDatabase(ClassDatabaseFile_sharedptr pClassDatabase);
	//Gets the class database (may return an empty pointer).
	ClassDatabaseFile_sharedptr GetClassDatabase();
...
public:
	...
	bool EnqueueContainersTask(class AppContext &appContext, std::shared_ptr<AssetsFileContextInfo> selfPointer);
...
class BundleFileContextInfo : public FileContextInfo
{
	...
	std::string getNewEntryName(size_t index, bool acquireLock);
public:
	...
	std::shared_ptr<IAssetsReader> makeEntryReader(size_t index, bool &readerIsModified);

	//Called once the bundle directory is deserialized.
	void onDirectoryReady(class AppContext &appContext);
	
	//Sets the override name for a bundle entry. Returns true on success.
	bool renameEntry(class AppContext &appContext, size_t index, std::string newEntryName);
	//Sets the override reader for a bundle entry. Returns true on success.
	bool overrideEntryReader(class AppContext &appContext, size_t index, std::shared_ptr<IAssetsReader> pReader, bool hasSerializedData);
	...
	bool removeEntry(class AppContext &appContext, size_t index);
	//Adds a new entry and returns its index. Returns (size_t)-1 on failure.
	size_t addEntry(class AppContext &appContext, std::shared_ptr<IAssetsReader> pReader, bool hasSerializedData, std::string entryName);
	
	//Checks whether an entry has been removed. Can return true even if the child file still is open.
	bool entryIsRemoved(size_t index);
	...
	bool entryHasChanged(size_t index);
...
public:
	...
	std::shared_ptr<DecompressTask> EnqueueDecompressTask(AppContext &appContext, std::shared_ptr<BundleFileContextInfo> &selfPointer, std::string outputPath);
...
public:
	ResourcesFileContextInfo(ResourcesFileContext *pContext, unsigned int fileID, unsigned int parentFileID);
	...
	bool setByReplacer(class AppContext& appContext, BundleReplacer* pReplacer);
	...
	void addResource(std::shared_ptr<IAssetsReader> pReader, uint64_t readerOffs, uint64_t size, uint64_t &resourcesFilePos);
	...
	std::shared_ptr<IAssetsReader> getResource(std::shared_ptr<ResourcesFileContextInfo> selfRef, uint64_t offs, uint64_t size);
};
...
class GenericFileContextInfo : public FileContextInfo
{
	...
	bool changedFlag;
public:
	GenericFileContextInfo(GenericFileContext *pContext, unsigned int fileID, unsigned int parentFileID);
