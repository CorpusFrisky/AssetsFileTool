...
//	EBundleFileOpenStatus OpenInsideTask(TaskProgressManager *pProgressManager, unsigned int initProgress = 0, unsigned int progressScale = 100);
//
//	void Close();
//	
...
class AssetsFileContext : public IFileContext
{
	...
	/*virtual*/ EAssetsFileOpenStatus OpenSync(bool makeBinaryTable, unsigned int initProgress, unsigned int progressScale);
public:
	...
	//Run the open task inside another task. Takes up 100 progress units, starting with initProgress. Requires progressScale >= initProgress + 100. 
	EAssetsFileOpenStatus OpenInsideTask(bool makeBinaryTable, unsigned int initProgress = 0, unsigned int progressScale = 100);
