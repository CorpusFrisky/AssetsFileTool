...
namespace std
{
    ...
    {
		std::size_t operator()(ContainerDependencyEntry const& pptr) const;
    };
    template<> struct hash<ContainerPPtr>
    {
		std::size_t operator()(ContainerPPtr const& pptr) const;
    };
}
...
struct equality_ContainerDependencyLookupEntry
{
	...
	equality_ContainerDependencyLookupEntry(std::vector<ContainerDependencyEntry> &dependencies);

	bool operator()(ContainerDependencyLookupEntry const& pptrref_a, ContainerDependencyLookupEntry const& pptrref_b) const;
...
struct hash_ContainerDependencyLookupEntry
{
	std::vector<ContainerDependencyEntry> *pDependencies;
	std::hash<ContainerDependencyEntry> entryHash;
	hash_ContainerDependencyLookupEntry(std::vector<ContainerDependencyEntry> &dependencies);

	std::size_t operator()(ContainerDependencyLookupEntry const& pptr) const;
};
...
public:
	AssetContainerList();
	AssetContainerList &operator=(AssetContainerList &&other);
	~AssetContainerList();
	inline void Clear()
	{
		containers.clear();
	...
	bool LoadFrom(AssetBundleAsset &fileTable);
	bool LoadFrom(ResourceManagerFile &resourceFile);
	...
	std::vector<const ContainerEntry*> getContainers(unsigned int fileID, long long pathID) const;
	std::vector<const ContainerEntry*> getParentContainers(unsigned int fileID, long long pathID) const;
};