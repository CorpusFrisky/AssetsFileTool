...
int AssetsFileContextInfo::ContainersTask::execute()
{
		...
		if (_GetMonoBehaviourScriptInfo(asset, appContext, fullScriptName, assemblyName, scriptName, namespaceName, scriptAsset))
		{
			...
			for (size_t i = 0; i < scriptDatabases.size(); i++)
			{
				...
				for (size_t k = 0; k < pCurDatabase->classes.size(); k++)
				{
					...
					if (curType.fields.size() > 1 && !_stricmp(curAssemblyName, assemblyName.c_str()) && !strcmp(curFullClassName, fullScriptName.c_str()))
					{
						if (pScriptID)
						{
	...
	if (classID >= 0 && scriptIndex == 0xFFFF)
	{
		...
		else
		{
			for (size_t i = 0; i < pAssetsFile->typeTree.fieldCount; i++)
			{
				...
				{
					if (!_strnicmp(pType->base.name, "Base", 5))
					{
...
int BundleFileContextInfo::DecompressTask::execute()
{
	...
	this->reopenStatus = pNewBundleContext->OpenInsideTask();
